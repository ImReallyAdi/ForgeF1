...
export default new NativeFunction({
  ...
  version: "1.0.0",
  description:
    "Returns a value from the GitHub repository event payload (e.g., rename)",
  brackets: true,
  args: [
    {
      name: "property",
      ...
      type: ArgType.String,
      required: false,
      description: "The property to retrieve from the repository event payload",
    },
  ],
  unwrap: true,
  ...
  async execute(ctx, [prop]) {
    ...
    if (ext.latestEventType !== "repository") return this.success("");
    if (!prop)
